...
class StudentRegistration(http.Controller):
    ...
    def registration_create(self, **kw):
        ...
        if request.session.get('is_data') and kw.get('check_data'):
            if kw.get('fname') and kw.get('lname'):
                name = self.remove(kw.get('fname')) + ' ' + \
                    self.remove(kw.get('lname'))
                old_partner = request.env['res.partner'].sudo().search(
                    ['|', ('name', '=', name), ('mobile', '=', self.remove(kw.get('mobile')))])
                if old_partner:
                    massage = 'Records is already available for same name or mobile number.\n'
                    return request.render('bi_sport_center_management.registration_create_massage', {'massage': massage, 'admission': admission})
                vals = {'name': name, 'email': self.remove(kw.get('email')),
                        'mobile': self.remove(kw.get('mobile')),
                        'phone': self.remove(kw.get('parent_mobile')),
                        'street': kw.get('address'), 'street2': kw.get('address2'),
                        'zip': kw.get('zip'), 'state_id': kw.get('state'),
                        'city': kw.get('city'),
                        'p_name': self.remove(kw.get('parent_fullname')),
                        'is_disability': kw.get('is_disability'),
                ...
                if partner:
                    values = {'student_id': partner.id,
                            ...
                            'sport_id': kw.get('sport_id'),
                            'duration': kw.get('duration'),
                            'check_register':True,
                            }
                    if values:
                        request.session['is_data'] = False
                        admission = request.env['student.admission'].sudo().create(
                            values)
                   
        if admission:
            massage = f'Your Registration Request {admission.name} is successfully Generated.'
            return request.render('bi_sport_center_management.registration_create_massage', {'massage': massage, 'admission': admission})
        else:
            return request.render('bi_sport_center_management.registration_create_massage', {'massage': massage, 'admission': admission})
    ...
    def inquiry_create(self, **kw):
        ...
        if request.session.get('is_data'):
            ...
            if kw.get('fname') and kw.get('lname'):
                name = self.remove(kw.get('fname')) + ' ' + \
                    self.remove(kw.get('lname'))
                sport_id_val = request.env['student.admission'].sudo().browse(int(kw.get('sport_id')))
                values = {'first_name': self.remove(kw.get('fname')),
                          'last_name': self.remove(kw.get('lname')),
                          'mobile': self.remove(kw.get('mobile')),
                          'email': self.remove(kw.get('email')),
                          'parent_mobile': self.remove(kw.get('parent_mobile')),
                          'p_name': self.remove(kw.get('parent_fullname')),
                          'sport_id': sport_id_val.ids,
                          'level_id': kw.get('level_id'),
                          'duration': kw.get('duration'),
                          'query': kw.get('query'),
                          }
                if values:
                    ...
                    request.session['is_data'] = False
            if inquiry:
                massage = f'Your Registration Request {inquiry.name} is successfully Generated.'
    ...
    def registration(self, **kw):
        ...
        centers = request.env['res.partner'].sudo().search(
            [('is_sport', '=', True)])
        studies = request.env['product.product'].sudo().search(
            [('is_sportname', '=', True)])
        return request.render('bi_sport_center_management.registration', {'states': states, 'studies': studies, 'centers' :centers})
    ...
    def confirm_registration(self, id, **kw):
        ...
        if admission:
            massage = f'Your admission Request {admission.name} is successfully Generated.'
            return request.render('bi_sport_center_management.registration_create_massage', {'massage': massage, 'admission': admission})
        else:
            return request.render('bi_sport_center_management.registration_create_massage', {'massage': massage, 'admission': admission})
    ...
    def check_book_availability(self,**kw):
        data = {}
        tz = pytz.timezone(request.env.user.tz) or pytz.utc
        start_date = datetime.fromisoformat(kw.get('start_date'))
        end_date = datetime.fromisoformat(kw.get('end_date'))
    ...
    def book_ground(self, **kw):
        ...
        sports = request.env['product.product'].sudo().search(
        [('is_sportname', '=', True)])
